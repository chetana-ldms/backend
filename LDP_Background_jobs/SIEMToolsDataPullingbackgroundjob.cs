...
{
    public class SIEMToolsDataPullingbackgroundjob : BackgroundService
    {
        //TODO
        //_period should make configurable 

        private readonly TimeSpan _period = TimeSpan.FromMilliseconds(1000);
       // private readonly ILogger<SIEMToolsDataPullingbackgroundjob> _logger;
        ...
        public SIEMToolsDataPullingbackgroundjob(
        IApplicationLogBL logger,
       ...
       // , IConfigurationDataBL configData
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.AddLogInforation($"SentinalOne Job Intiated.. at {DateTime.UtcNow}", Constants.Logger_severity_Information);
               ...
               // && await timer.WaitForNextTickAsync(stoppingToken))
            {
                try
                {
                    if (IsEnabled)
                    {
                        ...
                        SIEMToolsDataPullingService SIEMToolService = asyncScope.ServiceProvider.GetRequiredService<SIEMToolsDataPullingService>();
                        await SIEMToolService.DoSomethingAsync();

                        // Sample count increment
                        _executionCount++;
                        //_logger.LogInformation(
                        //    $"Executed SIEMToolsDataPullingService - Count: {_executionCount}");
                    ...
                    {
                        //_logger.LogInformation(
                        //    "Skipped SIEMToolsDataPullingService");
                        _logger.AddLogInforation($"SSkipped SIEMToolsDataPullingService.. at {DateTime.UtcNow}", Constants.Logger_severity_Information);
                ...
                {
                    //_logger.LogInformation(
                    //    $"Failed to execute SIEMToolsDataPullingService with exception message {ex.Message}. Good luck next round!");
                    _logger.AddLogError($"Failed to execute SIEMToolsDataPullingService with exception message {ex.Message}. Good luck next round!"
                            , Constants.Logger_severity_Error);
